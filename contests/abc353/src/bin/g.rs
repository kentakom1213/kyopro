...
use crate::{monoid::examples::Max, segment_tree::SegmentTree};
...
fn main() {
    ...
    let mut pre = SegmentTree::<Max>::new(N);
    let mut suf = SegmentTree::<Max>::new(N);

    *pre.get_mut(0).unwrap() = 0;
    *suf.get_mut(0).unwrap() = 0;
    ...
    for &(t, p) in &TP {
        ...
        let nxt = (-C * T + pre.get_range(..=t)).max(C * T + suf.get_range(t..)) + p;

        chmax!(ans, nxt);

        *pre.get_mut(t).unwrap() = nxt + C * T;
        *suf.get_mut(t).unwrap() = nxt - C * T;
    }
