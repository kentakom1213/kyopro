...
use cp_library_rs::rolling_hash::{self, RollingHash};
...
fn main() {
    ...
    let rh = RollingHash::from_str(&S, BASE);
    ...
    for t in 0..505050 {
        ...
        for &q in &qs[t] {
            ...
            let thash = RollingHash::from_str(&tt, BASE).get(0, tt.len());

            ans[q] = *cnt.get(&thash).unwrap_or(&0);
        }
    }
