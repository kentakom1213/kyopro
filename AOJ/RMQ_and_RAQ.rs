...
fn main() {
    ...
    // 遅延セグメント木
    let mut seg = LazySegmentTree::<RMQandRAQ>::new(N);

    // クエリの処理
    for _ in 0..Q {
        let query = get!(isize;;);

        if let &[1, s, t] = &query[..3] {
            // 取得
            let res = seg.get_range(s as usize, (t + 1) as usize);
            println!("{}", res);
        }
        else if let &[0, s, t, x] = &query[..4] {
            // 区間加算
            seg.set_range(s as usize, (t + 1) as usize, x);
        }
