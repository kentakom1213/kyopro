...
fn main() {
    ...
    let NG = CD.iter().fold(vec![vec![]; N], |mut v, &(c, d)| {
        v[c].push(d);
        v
    });

    let mut seg = SegmentTree::<Alg::Max>::from(&B);
    ...
    for (c, &a) in A.iter().enumerate() {
        ...
        let mut old = vec![];
        for &d in &NG[c] {
            let mut tmp = seg.get_mut(d).unwrap();
            old.push(*tmp);
            *tmp = -INF;
        }
        // 最大値を取得
        ...
        // もとに戻す
        for (&d, &o) in NG[c].iter().zip(old.iter()) {
            *seg.get_mut(d).unwrap() = o;
        }
    }
