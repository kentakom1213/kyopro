...
fn rec(u: usize, G: &Vec<Vec<usize>>, visited: &mut Vec<bool>, dp: &mut Vec<Vec<usize>>) {
    if visited[u] {
        return;
    }
    visited[u] = true;
    ...
    for &v in &G[u] {
        if !visited[v] {
            rec(v, G, visited, dp);
            // 白に塗る
            dp[u][0] = dp[u][0].mmul(dp[v][0].madd(dp[v][1]));
            // 黒に塗る
            dp[u][1] = dp[u][1].mmul(dp[v][0]);
        }
    }
    debug!(u + 1, &dp);
}
