...
static SEED: [u8; 32] = [2, 0, 0, 2, 1, 2, 1, 3,
                         ...
                         2, 0, 1, 0, 0, 8, 2, 1,
                         2, 0, 1, 3, 0, 3, 2, 2];
static SHUFFLE_COUNT_BUILD: usize = 20;
static SHUFFLE_COUNT_QUERY: usize = 5;
static SHUFFLE_OFFSET: usize = 4;
...
fn main() {
    ...
    for i in 0..M {
        ...
        // `SHUFFLE_COUNT_BUILD`回行う
        for _ in 0..SHUFFLE_COUNT_BUILD {
            // クローンしたグラフ
    ...
    for _ in 0..100 {
        ...
        let mut degs = vec![0; V];
        // ランダムに変化させる
        for _ in 0..SHUFFLE_COUNT_QUERY {
            let mut g = H.clone();
            ...
            for (i, v) in g.deg_list().iter().enumerate() {
                degs[i] += v;
            }
        }

        for v in degs.iter_mut() {
            *v *= SHUFFLE_OFFSET;
        }     
