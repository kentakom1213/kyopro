...
const SHUFFLE_COUNT: usize = 10;  // 100;
const SEED: [u8; 32] = [2, 0, 0, 2, 1, 2, 1, 3,
                        2, 0, 0, 4, 0, 9, 1, 0,
                        2, 0, 1, 0, 0, 8, 2, 1,
                        2, 0, 1, 3, 0, 3, 2, 2];
const SPLIT_RATE: usize = 20;  // `V / SPLIT_RATE`個の点を切り離す
...
fn main() {
    ...
    for i in 0..M {
        ...
        // ランダムに選んだ点をグラフから切り離す
        if i % 3 == 1 {
            let mut verbs: Vec<usize> = (0..V/3).collect();
            verbs.shuffle(&mut rng);

            for &u in &verbs[..SPLIT_VERB] {
                graph.split_point(u);
                graph.split_point(u+1);
                graph.split_point(u+2);
            }
        }
        // ランダムに選んだ点をグラフから切り離す
        if i % 3 == 2 {
            ...
            for &u in &verbs[..SPLIT_VERB] {
                graph.merge_point(u);
                graph.merge_point(u+1);
                graph.merge_point(u+2);
            }
        }
