...
int main() {
    ...
    for (int i=0; i < LOOP_COUNT; i++) {
        ...
        // 条件を満たさない場合、元に戻す
        if (is_not_contain_center || is_not_positive_area || is_in_collision) {
            adds[choose].reset(reset);
        }

        // 変更後の面積
        int after_area = adds[choose].area();
        prio -= after_area - before_area;

        // キューに追加
        q.push(make_pair(prio, choose));
    }
