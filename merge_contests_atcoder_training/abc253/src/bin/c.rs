...
fn main() {
    ...
    let mut multiset = splay_tree::Multiset::new();
    ...
    for _ in 0..Q {
        ...
        match t {
            ...
            2 => {
                ...
                let cnt = multiset.count(&x);
                for _ in 0..cnt.min(c) {
                    multiset.delete(&x);
                }
            }
            _ => {
                multiset.lower_bound(&0);
                let min = multiset.root.as_ref().unwrap().key;
                multiset.upper_bound(&INF);
                let max = multiset.root.as_ref().unwrap().key;
                println!("{}", max - min);
            }
        }
    }
