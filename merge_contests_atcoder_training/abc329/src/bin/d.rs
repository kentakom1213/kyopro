...
pub mod Alg {
    ...
    pub struct Max;
    impl Monoid for Max {
        type Val = (usize, isize); // (得票, -インデックス)
        const E: Self::Val = (0, isize::MIN);
        fn op(left: &Self::Val, right: &Self::Val) -> Self::Val {
            *left.max(right)
        }
    }
...
fn main() {
    ...
    let mut seg = SegmentTree::<Alg::Max>::from(&(0..N as isize).map(|i| (0, -i)).collect());

    for &v in &A {
        // 更新
        seg.get_mut(v).unwrap().0 += 1;

        // 最大値を取得
        let (vote, idx) = seg.get_range(..);
